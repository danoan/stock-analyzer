...
def test_fetch_info_success():
    with patch("study_notebook.core.api.httpx.Client", _mock_http(_INFO_RESPONSE)):
...
def test_fetch_info_wrong_type_raises():
    ...
    with patch("study_notebook.core.api.httpx.Client", _mock_http(body)):
...
def test_fetch_info_http_error_propagates():
    ...
    mock_client_cls = MagicMock()
    mock_client_cls.return_value.__enter__.return_value.post.return_value = mock_resp
    with patch("study_notebook.core.api.httpx.Client", mock_client_cls):
...
def test_lookup_tickers_success():
    with patch("study_notebook.core.api.httpx.Client", _mock_http(_LOOKUP_RESPONSE)):
...
def test_lookup_tickers_empty_results():
    with patch("study_notebook.core.api.httpx.Client", _mock_http({"results": []})):
...
def test_get_available_metrics_returns_sorted_numeric_keys():
    ...
    with patch("study_notebook.core.api.fetch_info", return_value=info):
...
def test_realize_analysis_normalize_score():
    ...
    with patch("study_notebook.core.api.fetch_info", side_effect=mock_fetch):
...
def test_realize_analysis_fetch_error_captured():
    ...
    with patch("study_notebook.core.api.fetch_info", side_effect=Exception("timeout")):
        results = realize_analysis(analysis, ["AAPL"])
