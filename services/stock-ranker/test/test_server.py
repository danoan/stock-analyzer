...
from stock_ranker.server.app import app
...
async def test_realize_with_tickers(client: httpx.AsyncClient):
    ...
    with patch("stock_ranker.server.app.realize_analysis", return_value=fake_results) as mock:
        r = await client.post(
...
async def test_realize_upstream_error_502(client: httpx.AsyncClient):
    a = await _create_analysis(client)
    with patch(
        "stock_ranker.server.app.realize_analysis", side_effect=RuntimeError("upstream down")
    ):
        r = await client.post(
            f"/analyses/{a['id']}/realize",
            json={"tickers": ["AAPL"], "collections": [], "force_refresh": False},
...
async def test_tickers_lookup_success(client: httpx.AsyncClient):
    ...
    with patch("stock_ranker.server.app.lookup_tickers", return_value=fake) as mock:
...
async def test_tickers_lookup_upstream_error_502(client: httpx.AsyncClient):
    with patch(
        "stock_ranker.server.app.lookup_tickers", side_effect=RuntimeError("network error")
    ):
...
async def test_metrics_success(client: httpx.AsyncClient):
    ...
    _target = "stock_ranker.server.app.get_available_metrics"
...
async def test_metrics_upstream_error_502(client: httpx.AsyncClient):
    with patch(
        "stock_ranker.server.app.get_available_metrics",
