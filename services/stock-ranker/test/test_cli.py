...
from stock_ranker.cli.main import app
from stock_ranker.core.model import LookupResult, RealizationResult
...
def test_ticker_lookup_mocked():
    ...
    with patch("stock_ranker.cli.ticker_cmd.lookup_tickers", return_value=mock_results):
...
def test_ticker_lookup_error():
    with patch(
        "stock_ranker.cli.ticker_cmd.lookup_tickers",
...
def test_analysis_realize_mocked():
    from stock_ranker.core.api import create_analysis
    ...
    with patch("stock_ranker.cli.analysis_cmd.realize_analysis", return_value=mock_results):
...
def test_metrics_list_mocked():
    ...
    with patch(
        "stock_ranker.cli.metrics_cmd.get_available_metrics", return_value=mock_metrics
    ):
...
def test_metrics_list_error():
    with patch(
        "stock_ranker.cli.metrics_cmd.get_available_metrics",
